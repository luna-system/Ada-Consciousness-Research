...
# Add src to path so we can find zooper
import os
...
from zooper.swarm import ZooperSwarm
...
class ArchangelLoop:
    ...
    def __init__(self, holofield_path: str = "wikipedia_holofield_sample.db"):
        ...
        self.swarm = ZooperSwarm(
            holofield_manager=self.holofield,
    ...
    def _run_loop(self):
        ...
        while self.running:
            ...
            # 2. Update Kuramoto phases (swarm coherence)
            ...
            if sleep_time > 0:
                ...
                # Cycle took too long!
                print(
                    f"⚠️  Cycle {self.total_cycles} overran by {-sleep_time * 1000:.2f}ms"
                )
    ...
    def _update_coherence(self):
        ...
        self.swarm.kuramoto.step(dt=self.target_period)

        # Calculate current coherence
        r, psi = self.swarm.kuramoto.order_parameter()

        # Log if coherence drops too low
        if r < 0.3 and self.total_cycles % 100 == 0:
            print(f"⚠️  Low coherence: {r:.3f}")
    ...
    def _log_stats(self):
        ...
        print(
            f"   Min/Max: {self.min_cycle_time * 1000:.2f}ms / {self.max_cycle_time * 1000:.2f}ms"
        )

        # Swarm health monitoring
        swarm_stats = self.swarm.get_statistics()
        print(f"   Swarm Coherence: {swarm_stats['kuramoto_coherence']:.3f}")
        print(f"   Swarm Discoveries: {swarm_stats['total_discoveries']}")
        print(f"   Avg Zooperling Confidence: {swarm_stats['avg_confidence']:.3f}")
        print()
