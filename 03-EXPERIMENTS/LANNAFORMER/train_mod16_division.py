...
class ModularAdditionDataset(Dataset):
    """
    Dataset for modular division: (a / b) mod p
    ...
    def __init__(self, modulus: int = 16, train: bool = True, train_fraction: float = 0.5):
        ...
        for a in range(modulus):
            for b in range(modulus):
                result = (a * mod_inverse(b, modulus) if b != 0 else 0) % modulus
...
def main():
    """Main training loop"""
    print("🌌 Training LANNAformer on Mod 16 DIVISION")
    ...
    # Results directory
    results_dir = Path(f"mod16_division_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
