...
def test_color_cuts(data_dir='data/ztf_dr2/ztfsniadr2_lite/tables'):
    """Test multiple color cut combinations with Bayesian comparison."""
    ...
    for name, gz_thresh, mass_thresh, coeval_tol in thresholds:
        ...
        try:
            ...
            if lcdm_results and nonaccel_results:
                ...
                # Bayesian comparison
                bayes = compare_models_bayesian(lcdm_results, nonaccel_results, n_young)
    ...
    logger.info("=" * 70)
    logger.info(f"{'Cut':<15} {'g-z':<6} {'Mass':<6} {'N_young':<9} {'N_coeval':<9} {'Δχ²':<10} {'ΔBIC':<10} {'Winner':<12} {'Evidence'}")
    logger.info("-" * 70)
    ...
    for r in results:
        ...
        logger.info(f"{r['name']:<15} {r['gz_threshold']:<6.1f} {r['mass_threshold']:<6.1f} {r['n_young']:<9} {r['n_coeval']:<9} {delta_chi2_str:<10} {delta_bic_str:<10} {winner:<12} {evidence}")
...
if __name__ == "__main__":
    ...
    df_results.to_csv('data/colorcut_validation_results_bayesian.csv', index=False)
    logger.info("💾 Results saved to data/colorcut_validation_results_bayesian.csv")
