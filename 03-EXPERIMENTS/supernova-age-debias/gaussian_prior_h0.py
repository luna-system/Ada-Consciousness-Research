...
def chi2_lcdm(params):
    ...
    # Add Gaussian prior penalty
    prior_penalty = -2 * log_prior_h0(H0)  # Convert to chi2 scale
...
# Compute residuals for both models with common sigma
...
# AIC and BIC (include prior penalty in parameter count?)
# For BIC, we count free parameters: LCDM has 4 (H0, Om, OL, M), Non-Accel has 3 (H0, q0, M)
# The Gaussian prior doesn't add a free parameter, it constrains one
...
print("=" * 70)
print("GAUSSIAN PRIOR RESULTS (H0 ~ N(73.0, 1.4) from SH0ES)")
print("=" * 70)
...
print()
print(f"H0 prior: N({H0_prior_mean}, {H0_prior_std})")
print(f"LCDM H0 pulls: {(H0_l - H0_prior_mean) / H0_prior_std:.2f} sigma")
print(f"Non-Accel H0 pulls: {(H0_n - H0_prior_mean) / H0_prior_std:.2f} sigma")
print("=" * 70)
